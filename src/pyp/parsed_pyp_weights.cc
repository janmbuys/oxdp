...
ParsedPypWeights<tOrder, aOrder>::ParsedPypWeights(boost::shared_ptr<Dict> dict,
    ...
    : PypWeights<tOrder>(dict->tag_size()),
      shre_lm_(2, 1, 1, 1, 1),
      action_lm_(num_actions, 1, 1, 1, 1),  //num_actions - 1
      num_actions_(num_actions) {}

template <unsigned tOrder, unsigned aOrder>
Real ParsedPypWeights<tOrder, aOrder>::predictWord(WordId word,
...
template <unsigned tOrder, unsigned aOrder>
Reals ParsedPypWeights<tOrder, aOrder>::predictWordOverTags(
    WordId word, Context context) const {
  return Reals(numTags(), 0);
}

template <unsigned tOrder, unsigned aOrder>
Real ParsedPypWeights<tOrder, aOrder>::predictTag(WordId tag,
                                                  Context context) const {
...
template <unsigned tOrder, unsigned aOrder>
Real ParsedPypWeights<tOrder, aOrder>::predictAction(WordId action,
                                                     Context context) const {
  /*if (action == 0) {
...
template <unsigned tOrder, unsigned aOrder>
void ParsedPypWeights<tOrder, aOrder>::updateInsert(
    const boost::shared_ptr<ParseDataSet>& examples, MT19937& eng) {
  ...
  for (unsigned i = 0; i < examples->action_example_size(); ++i) {
    /*if (examples->action_at(i) == 0)
       ...
       action_lm_.increment(examples->action_at(i) - 1,
     examples->action_context_at(i).words, eng);
     } */
    action_lm_.increment(examples->action_at(i),
                         examples->action_context_at(i).words, eng);
  }
}
...
template <unsigned tOrder, unsigned aOrder>
void ParsedPypWeights<tOrder, aOrder>::updateRemove(
    const boost::shared_ptr<ParseDataSet>& examples, MT19937& eng) {
  ...
  for (unsigned i = 0; i < examples->action_example_size(); ++i) {
    /*if (examples->action_at(i) == 0)
      ...
      action_lm_.decrement(examples->action_at(i) - 1,
    examples->action_context_at(i).words, eng);
    } */
    action_lm_.decrement(examples->action_at(i),
                         examples->action_context_at(i).words, eng);
  }
}
