...
Weights::Weights(const boost::shared_ptr<ModelConfig>& config,
...
size_t Weights::numParameters() const { return size; }

void Weights::getGradient(const boost::shared_ptr<DataSet>& examples,
  ...
  MatrixReal weighted_representations =
      getWeightedRepresentations(examples, prediction_vectors, word_probs);

  getFullGradient(examples, contexts, context_vectors, prediction_vectors,
                  weighted_representations, word_probs, gradient, words);
}

void Weights::getContextVectors(const boost::shared_ptr<DataSet>& examples,
                                vector<WordsList>& contexts,
  ...
  context_vectors.resize(context_width,
                         MatrixReal::Zero(word_width, examples->size()));
...
void Weights::setContextWords(const vector<WordsList>& contexts,
...
MatrixReal Weights::getPredictionVectors(
  ...
  // Compute forward pass (used during training).
  for (int i = 0; i < context_width; ++i) {
...
MatrixReal Weights::getContextProduct(int index,
                                      const MatrixReal& representations,
...
void Weights::getFullGradient(const boost::shared_ptr<DataSet>& examples,
...
void Weights::getContextGradient(
    size_t prediction_size, const vector<WordsList>& contexts,
    const vector<MatrixReal>& context_vectors,
    const MatrixReal& weighted_representations,
  ...
  for (int j = 0; j < context_width; ++j) {
    ...
    if (config->diagonal_contexts) {
      gradient->C[j] += context_vectors[j]
                            .cwiseProduct(weighted_representations)
                            .rowwise()
                            .sum();
    } else {
      gradient->C[j] +=
          weighted_representations * context_vectors[j].transpose();
    }
  }
}

bool Weights::checkGradient(const boost::shared_ptr<DataSet>& examples,
                            const boost::shared_ptr<Weights>& gradient,
                            double eps) {
...
Real Weights::getObjective(const boost::shared_ptr<DataSet>& examples) const {
  vector<WordsList> contexts;
  vector<MatrixReal> context_vectors;
  MatrixReal prediction_vectors;
  MatrixReal word_probs;
...
Real Weights::getObjective(const boost::shared_ptr<DataSet>& examples,
                           vector<WordsList>& contexts,
                           vector<MatrixReal>& context_vectors,
                           MatrixReal& prediction_vectors,
...
void Weights::estimateProjectionGradient(
  ...
  for (size_t i = 0; i < examples->size(); ++i) {
    ...
    Real log_pos_prob =
        R.col(word_id).dot(prediction_vectors.col(i)) + B(word_id);
    Real pos_prob = exp(log_pos_prob);
    assert(pos_prob <= numeric_limits<Real>::max());

    Real pos_weight = (noise_samples * unigram(word_id)) /
                      (pos_prob + noise_samples * unigram(word_id));
    ...
    for (int j = 0; j < noise_samples; ++j) {
      ...
      Real log_neg_prob = R.col(noise_word_id).dot(prediction_vectors.col(i)) +
                          B(noise_word_id);
      Real neg_prob = exp(log_neg_prob);
      assert(neg_prob <= numeric_limits<Real>::max());

      Real neg_weight =
          neg_prob / (neg_prob + noise_samples * unigram(noise_word_id));
...
void Weights::estimateGradient(const boost::shared_ptr<DataSet>& examples,
                               const boost::shared_ptr<Weights>& gradient,
                               Real& objective, MinibatchWords& words) const {
  vector<WordsList> contexts;
  vector<MatrixReal> context_vectors;
...
void Weights::syncUpdate(const MinibatchWords& words,
                         const boost::shared_ptr<Weights>& gradient) {
...
void Weights::updateSquared(const MinibatchWords& global_words,
...
void Weights::updateAdaGrad(const MinibatchWords& global_words,
                            const boost::shared_ptr<Weights>& global_gradient,
  ...
  W.segment(block.first, block.second) -=
      global_gradient->W.segment(block.first, block.second)
          .binaryExpr(adagrad->W.segment(block.first, block.second),
...
void Weights::clear(const MinibatchWords& words, bool parallel_update) {
  if (parallel_update) {
    ...
    Block block =
        getBlock(Q.size() + R.size(), W.size() - (Q.size() + R.size()));
...
VectorReal Weights::getPredictionVector(const Context& context) const {
  ...
  // Compute forward pass at test time.
...
Real Weights::predict(int word, Context context) const {
  ...
  //TODO Decide if it is worth using the normalizer cache.
  // auto ret = normalizerCache.get(context.words);
  // if (ret.second) {
  //  prob = (R.col(word).dot(prediction_vector) + B(word) - ret.first);
  //} else {
  Real normalizer = 0;
  VectorReal word_probs =
      logSoftMax(R.transpose() * prediction_vector + B, normalizer);
...
Reals Weights::predictViterbi(Context context) const {
  ...
  // Here computing a softmax is too expensive.
  VectorReal prediction_vector = getPredictionVector(context);
  VectorReal word_probs = R.transpose() * prediction_vector + B;
  for (int cw = 0; cw < num_words; ++cw) {
    int word_id = word_probs.maxCoeff();
    Real word_prob = word_probs(word_id);
    word_probs(word_id) = std::numeric_limits<Real>::min();

    probs[word_id] = -log(word_prob);
  }

  return probs;
}

int Weights::vocabSize() const { return config->vocab_size; }

void Weights::clearCache() { normalizerCache.clear(); }

MatrixReal Weights::getWordVectors() const { return R; }

MatrixReal Weights::getFeatureVectors() const { return Q; }

bool Weights::operator==(const Weights& other) const {
  return *config == *other.config && *metadata == *other.metadata &&
         size == other.size && W == other.W;
}

Weights::~Weights() { delete data; }
