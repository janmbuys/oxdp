...
void TaggedParsedFactoredMetadata::initialize(const boost::shared_ptr<ParsedCorpus>& corpus) {
...
VectorReal TaggedParsedFactoredMetadata::getTagBias() const {
  return tagBias;
}

bool TaggedParsedFactoredMetadata::operator==(const TaggedParsedFactoredMetadata& other) const {
  return Metadata::operator==(other)
      && tagBias == other.tagBias;
}
