...
// lm.print(cerr);
  ...
  for (unsigned j = 0; j < test.size(); ++j) {
    ...
    vector<float> particle_liw; //importance weights
    particle_liw.resize(nPARTICLES);
    vector<float> particle_lp;
    ...
    for (auto& parser: particles) {
        ...
        cout << "  Importance weight: " << (-shift_llh * log(2) / log(10));
        cout << "  Sample weight: " << (-part_llh * log(2) / log(10)) << endl;
