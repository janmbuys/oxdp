...
// Class for computing various accuracy measures of the parsing models.
class AccuracyCounts {
 public:
  AccuracyCounts(boost::shared_ptr<Dict> dict);

  void inc_reduce_count() { ++reduce_count_; }

  void inc_reduce_gold() { ++reduce_gold_; }

  void inc_shift_count() { ++shift_count_; }

  void inc_shift_gold() { ++shift_gold_; }

  void inc_final_reduce_error_count() { ++final_reduce_error_count_; }

  void inc_complete_sentences() { ++complete_sentences_; }

  void inc_complete_sentences_lab() { ++complete_sentences_lab_; }

  void inc_complete_sentences_nopunc() { ++complete_sentences_nopunc_; }

  void inc_complete_sentences_lab_nopunc() { ++complete_sentences_lab_nopunc_; }

  void inc_gold_more_likely_count() { ++gold_more_likely_count_; }

  void inc_root_count() { ++root_count_; }

  void inc_directed_count() { ++directed_count_; }

  void inc_tag_count() { ++tag_count_; }

  void inc_directed_count_lab() { ++directed_count_lab_; }

  void inc_directed_count_nopunc() { ++directed_count_nopunc_; }

  void inc_directed_count_lab_nopunc() { ++directed_count_lab_nopunc_; }

  void inc_undirected_count() { ++undirected_count_; }

  void inc_undirected_count_nopunc() { ++undirected_count_nopunc_; }

  void inc_unheaded_count() { ++unheaded_count_; }

  void inc_unheaded_count_nopunc() { ++unheaded_count_nopunc_; }

  void inc_num_sentences() { ++num_sentences_; }

  void inc_total_length_nopunc() { ++total_length_nopunc_; }

  void inc_total_length_nopunc_headed() { ++total_length_nopunc_headed_; }

  void inc_total_length_punc_headed() { ++total_length_punc_headed_; }

  void add_likelihood(Real l) { likelihood_ += l; }
  ...
  void add_total_length(int l) { total_length_ += l; }

  void add_total_length_punc(int l) { total_length_punc_ += l; }

  void add_num_actions(int l) { num_actions_ += l; }

  void add_directed_count(int l) { directed_count_ += l; }

  void add_directed_count_nopunc(int l) { directed_count_nopunc_ += l; }

  void add_undirected_count(int l) { undirected_count_ += l; }

  void add_undirected_count_nopunc(int l) { undirected_count_nopunc_ += l; }

  void parseCountAccuracy(const Parser& prop_parse,
                          const ParsedSentence& gold_parse);

  void transitionCountAccuracy(const TransitionParser& prop_parse,
                               const ParsedSentence& gold_parse);

  void countAccuracy(const ArcStandardLabelledParser& prop_parse,
                     const ParsedSentence& gold_parse);
  ...
  Real tag_accuracy() const { return (tag_count_ + 0.0) / total_length_punc_; }
  ...
  Real root_accuracy() const { return (root_count_ + 0.0) / num_sentences_; }
  ...
  Real reduce_recall() const { return (reduce_count_ + 0.0) / reduce_gold_; }

  Real shift_recall() const { return (shift_count_ + 0.0) / shift_gold_; }

  Real likelihood() const { return likelihood_; }
  ...
  int total_length() const { return total_length_; }

  int total_length_punc() const { return total_length_punc_; }

  int total_length_punc_headed() const { return total_length_punc_headed_; }

  int total_length_nopunc() const { return total_length_nopunc_; }

  int total_length_nopunc_headed() const { return total_length_nopunc_headed_; }

  Real final_reduce_error_rate() const {
    return (final_reduce_error_count_ + 0.0) / total_length_punc_;
  }

  Real cross_entropy() const { return likelihood_ / total_length_; }

  Real beam_cross_entropy() const { return beam_likelihood_ / total_length_; }

  Real cross_entropy_noeos() const { return likelihood_ / total_length_punc_; }
  ...
  Real perplexity() const { return std::exp(cross_entropy()); }

  Real beam_perplexity() const { return std::exp(beam_cross_entropy()); }
